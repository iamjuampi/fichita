...
export function PongGame({ isActive, onPlay, onScoreUpdate }: PongGameProps) {
    ...
    const initAudio = async () => {
      try {
        audioRef.current = new Audio(
          "https://audio.jukehost.co.uk/PZNJCVLCvrQvU8R0SBOvnBAxtZxLtQqI"
        );
  ...
  const updateGame = useCallback(() => {
    ...
    if (
      ...
      const hitPos =
        (state.ball.y - (state.playerPaddle.y + PADDLE_HEIGHT / 2)) /
        (PADDLE_HEIGHT / 2);
    ...
    if (
      ...
      state.ball.dx = -state.ball.dx;
      const hitPos =
        (state.ball.y - (state.aiPaddle.y + PADDLE_HEIGHT / 2)) /
        (PADDLE_HEIGHT / 2);
      state.ball.dy = hitPos * 5;
      state.ball.speed = Math.min(state.ball.speed + 0.1, 3); // Cap at 3x speed
    }
  ...
  if (!isActive) {
    return (
      <div className="fixed inset-0 h-screen w-screen bg-gradient-to-b from-lime-500 to-yellow-400 flex items-center justify-center">
  ...
  return (
    <div className="fixed inset-0 h-screen w-screen bg-gradient-to-b from-lime-500 to-yellow-400 flex flex-col items-center justify-center relative">
      {/* Game UI */}
      <div className="absolute top-8 left-4 right-4 flex justify-between items-center text-white z-10">
        <div className="text-lg font-bold text-shadow-medium">
          Score: {score}
        </div>
