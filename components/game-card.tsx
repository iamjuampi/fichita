...
export function GameCard({ game, isActive }: GameCardProps) {
  ...
  const [comments, setComments] = useState(() => generateMockComments(game.id))
      ...
      <div className="absolute right-4 bottom-20 flex flex-col items-center space-y-6 z-20">
        ...
        <div className="relative">
          <div className="h-12 w-12 rounded-full bg-accent flex items-center justify-center border-2 border-white">
            <span className="text-sm font-bold text-accent-foreground">{game.username.charAt(1).toUpperCase()}</span>
          </div>
          <Button
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <div className="h-8 w-8 rounded-full bg-accent flex items-center justify-center">
              <span className="text-xs font-bold text-accent-foreground">{game.username.charAt(1).toUpperCase()}</span>
            </div>
            <span className="font-semibold text-sm text-shadow-soft">{game.username}</span>
            {!isFollowing && (
              <Button
                onClick={handleFollow}
                variant="ghost"
          ...
          <p className="text-sm leading-relaxed max-w-xs text-shadow-soft">{game.caption}</p>
          {isActive && fichitasEarned > 0 && (
            <div className="flex items-center space-x-2">
